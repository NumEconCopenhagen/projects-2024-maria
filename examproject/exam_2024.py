...
class production_economy:
    ...
    def opt_cb(self,l, p1, p2, w=1):
        c1 = self.alpha * (w*l+self.T+self.pi_j(p1,w)+self.pi_j(p2,w))/p1
        c2 = (1-self.alpha) * (w*l+self.T+self.pi_j(p1,w)+self.pi_j(p2,w))/(p1+self.tau)
        return c1, c2
    
    #consumer optimal labour supply
    #objective function
    def obj_ls(self, l, p1, p2):
        c1, c2 = self.opt_cb(l,p1,p2)
        return - (np.log(c1**self.alpha * c2**(1-self.alpha)) - self.nu*l**(1+self.epsilon)/(1+self.epsilon))

    #solver
    def opt_ls(self, p1, p2):
        l_quess = 1
        sol = optimize.minimize(self.obj_ls, l_quess, args=(p1, p2), method='SLSQP')
        return sol.x[0]
    
    #EQ
    #Market clearing conditions (only 2)
    def clear_con(self, p):
        error = np.zeros(2)
        c1, c2 = self.opt_cb(self.opt_ls(p[0],p[1]), p[0], p[1])
    ...
    def EQ(self):
        x0 = np.array([0.1,0.1])
        res = optimize.root(self.clear_con, x0)
        output =  res.x
        return output

    
    # # Opmitize consumer with varying tau (and T but constrained through error) - input = [p1,p2,]
    # def clear_SWF_con(self, input, tau, T):
    #     error = np.zeros(3)
    #     p1, p2 = input
    #     self.T = T
    #     self.tau = tau
    #     c1, c2 = self.opt_cb(self.opt_ls(p1,p2), p1, p2)

    #     y1 = self.opt_firm(p1)[1]
    #     y2 = self.opt_firm(p2)[1]        

    #     # Define threee errors for marketclearing and constrain T
    #     error = np.array([c1-y1, c2-y2])
    #     return error
    ...
    #     res = optimize.root(self.clear_SWF_con, x0, args = (tau,T))
    #     p1, p2 = res.x
    #     l = self.opt_ls(p1,p2)
    #     c1, c2 = self.opt_cb(l, p1, p2)
